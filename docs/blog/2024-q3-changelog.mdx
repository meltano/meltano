...
All dbt packages, which use [the Meltano dbt utility extension](https://github.com/meltano/dbt-ext/), now depend on the `meltano-dbt-ext` package directly [from PyPI](https://pypi.org/project/meltano-dbt-ext/) instead of the GitHub repository. This change makes installing dbt packages faster and more reliable.
