...
We'll assume you have [Meltano installed](/meltano-open/installation) already. You can tell Meltano is installed and which version by running `meltano --version`
...
Now that you have your very own Meltano project, it's time to add [plugins](/concepts/plugins) to it. We're going to add an extractor for GitHub to get our data. An [extractor](/concepts/plugins#extractors) is responsible for pulling data out of any data source. In this case, we choose a specific one with the `--variant` option to make this tutorial easy to work with.
