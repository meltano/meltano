...
<p>
  If you're having trouble throughout this tutorial, you can always head over to
  the <a href="https://meltano.com/slack">Slack channel</a> to get help.
</p>
:::
...
<p>
  Installing dbt-postgres may{" "}
  <a href="https://github.com/dbt-labs/dbt-postgres/issues/96">
    require building <code>psycopg2</code> from source
  </a>
  . An easy workaround is to constrain <code>dbt-postgres</code> in the plugin's{" "}
  <code>pip_url</code> in your <code>meltano.yml</code> file:
</p>
...
<br />
The result of your configuration will look like this in your meltano.yml,
remember that sensitive configurations are in your .env file:
...
<p>
  If you encounter an error related to dbt not being to clean files outside of
  the project directory, you can add the following to your{" "}
  <code>meltano.yml</code> file:
</p>
