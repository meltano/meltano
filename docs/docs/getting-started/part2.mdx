...
To configure the plugin, look at the options by running `meltano config list target-postgres`:
...
<br />
Fill in the details for these four attributes, and set add_record_metadata to True by using the `meltano config set target-postgres ATTRIBUTE VALUE` command:
