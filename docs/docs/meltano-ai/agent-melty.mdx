...
sidebar_label: Agent Melty
sidebar_position: 11.6
---

# Agent Melty

At the core of any Meltano Cloud workspace is a git repository. With Agent Melty, we can help you to build in that source of truth with your agent, push it to Meltano Cloud, and the platform reconciles it. The UI's job shrinks to one thing it should own: secrets, which never pass through the agent or into git.
...
If your workspace pre-dates Agent Melty, you can install it as a plugin - it's called "Agent Melty (the AI Knowledge Kit)"
:::
...
* **`AGENTS.md`** is the router: it tells the agent to check the knowledge base before doing anything, and which doc in it applies to the task. `CLAUDE.md` just points to `AGENTS.md`, so the same convention works whichever editor you're in. A trimmed example of what it looks like:
...
* **`.claude/meltano_knowledge_base/`** is the shared reference: how taps and targets work, how to configure sources and destinations, how runs get built and debugged. Split into `meltano/` and `meltano_cloud/`, each fronted by an `index.md` topic map. The agent reads these directly: no setup, no tooling, just files.
* **`.claude/workspace_knowledge_base/`** is your repo's own notes: empty at first, filled in as you and the agent hit and solve things specific to this project (see [Capture what you learn](#capture-what-you-learn)). It isn't part of the seeded kit, and it's never overwritten by it.
...
:::tip Deploy is async, don't assume instant
