...
:::info Availability
Melty MCP is awaiting listing in Claude's connector directory, so the search step below won't find it yet. Claude Code and other MCP clients can connect today using the address in [Claude Code](#claude-code) below.
:::
...
3. Search for **Meltano** and select it.
4. Click **Connect**. A browser window opens asking you to sign in to Meltano Cloud.
...
Claude Code connects straight to the server address. Run this once, from anywhere:
...
Any tool that supports **remote MCP servers over HTTP with OAuth sign-in** can connect to the same address:

```
https://app.meltano.com/api/mcp
```

It will discover the sign-in step by itself - there's no token to copy and paste, and you should never be asked to create one.
...
| **It's asking you to sign in again** | Sign-ins expire. Reconnect Meltano in Claude's connector settings, or run `/mcp` in Claude Code, and sign in again. |
| **Claude says it can't see Meltano at all** | Check Meltano is switched on for that chat in the tools menu. In Claude Code or another client, check the address is exactly `https://app.meltano.com/api/mcp`. |
