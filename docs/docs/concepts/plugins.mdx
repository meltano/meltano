...
While this extra can be managed using [`meltano config`](/reference/command-line-interface#config) or environment variables like any other setting,
selection filers are typically specified using [`meltano el`](/reference/command-line-interface#el)'s `--select` and `--exclude` options.
...
meltano el tap-gitlab target-jsonl --select commits
meltano el tap-gitlab target-jsonl --exclude project_members
