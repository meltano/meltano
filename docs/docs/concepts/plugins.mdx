...
- **Stream-only pattern** (e.g., `"users"`): Selects the stream and all automatically included properties (see the [Singer Spec](https://hub.meltano.com/singer/spec/#metadata) for information about _available_ vs _automatically_ included fields.). Additional properties must be explicitly included or included via the `stream.*` pattern.
- **Stream.property pattern** (e.g., `"users.id"`, `"orders.total"`): Selects specific properties within a stream. The stream is automatically included when a property is selected.
...
- name: tap-gitlab
  select:
  - project_members      # Stream-only: includes all "automatic" properties
