...
The `select` extra defines **what properties within each stream** should be included in data extraction. It operates at the property level and determines which specific attributes of your data streams are selected for replication.
...
Rules indicating that an entity or property should be excluded are prefixed with an exclamation mark (`!`). [Unix shell-style wildcards](<https://en.wikipedia.org/wiki/Glob_(programming)#Syntax>) can be used in entity and property identifiers to match multiple entities and/or properties at once.
...
- [`meltano el`](/reference/command-line-interface#el) CLI options: `--select` and `--exclude`
- Default: `[]`
...
|----------|----------------|
| **Property-level**: Controls which properties within streams are included | **Stream-level**: Controls which entire streams are included/excluded |
| **Configuration-time**: Set during project configuration | **Runtime**: Typically set via CLI options (`--select`, `--exclude`) |
| **Purpose**: Define what data fields you want | **Purpose**: Filter which streams to process in a specific run |
...
- **Runtime filtering**: When you want to process only certain streams for a specific pipeline run
