...
- A `--merge-state` flag can be passed to merge or overwrite the existing state at the end of the run.
...
- `--merge-state` will merge or overwrite the existing state at the end of the run .
...
# run the pipelines in series, performing a merging of state at the end of run.
meltano --environment=dev run --merge-state tap-gitlab target-postgres tap-salesforce target-mysql ...
```
