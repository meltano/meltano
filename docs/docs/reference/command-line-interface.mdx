...
The whole definition is compared, with every list sorted first. A new order of `capabilities` or settings is not an update, but a new label, description, or logo is. The definitions are cached for an hour, so a check makes a request at most once an hour for each plugin.
...
The JSON output reports `update_status` as `available` or `latest`. A plugin that is not checked reports `null`.
