...
The `INHERITS` column names the plugin that a plugin [inherits from](/concepts/plugins#plugin-inheritance), and the `CUSTOM` column marks a [custom plugin](/concepts/project#custom-plugin-definitions), meaning one that carries its own definition in `meltano.yml` rather than taking one from [Meltano Hub](https://hub.meltano.com/). A plugin from Meltano Hub has neither.

A plugin that inherits takes its parent's variant unless it names one of its own, and the `VARIANT` column shows `(inherited)` where it does. The JSON output reports the resolved variant, and an `inherited_variant` flag beside it.
...
```
 TYPE        NAME                  VARIANT       INHERITS     CUSTOM
 ───────────────────────────────────────────────────────────────────
