...
Lists the plugins in your project, along with the [variant](/concepts/plugins#variants) in use and the version that is installed.

By default, only plugins that are actually installed are listed, meaning those with a virtual environment in the project's `.meltano` directory. Pass `--available` to also list the plugins your project defines that have not been installed yet.

The version reported is the version of the distribution that was installed into the plugin's virtual environment, which is not necessarily the version requested by the plugin's `pip_url`. It is shown as `-` when the plugin was installed from something the version cannot be read from.
...
# Also list plugins that are defined but not installed
meltano plugin list --available
...
```
 ...
 loader      target-jsonl             andyh1203   0.1.4
```
