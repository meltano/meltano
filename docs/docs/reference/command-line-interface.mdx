...
Lists the plugins that Meltano Hub offers. Each plugin is listed once, on the [variant](/concepts/plugins#variants) that Meltano installs when no variant is specified. Pass `--all` to list every variant of every plugin.
...
With `--all`, a plugin takes one row for each variant it offers:
...
A plugin is named once, however many variants follow it. The JSON output repeats the type and the name on every record.

The JSON output carries the same fields in the same order:
...
Meltano Hub indexes one plugin type at a time, so listing every type makes a request for each of them. Pass `--plugin-type` to make a single request.
...
The JSON output carries the same fields in the same order, and the `pip_url` as well:
