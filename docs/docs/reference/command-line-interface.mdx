...
Lists every plugin that your project defines, along with the [variant](/concepts/plugins#variants) in use, the version that is installed, and whether the plugin is installed at all. A plugin is installed when it has a virtual environment in the project's `.meltano` directory. Installing the plugin creates that environment. `meltano add` installs a plugin as it adds it, and `meltano install` installs one later.

The version reported is the version of the distribution that was installed into the plugin's virtual environment, which is not necessarily the version requested by the plugin's `pip_url`. It is shown as `-` when the plugin is not installed, or when it was installed from something the version cannot be read from.
...
```
 TYPE        NAME                     VARIANT     VERSION STATE
 ──────────────────────────────────────────────────────────────
 extractor   tap-github               meltanolabs 1.0.3   installed
 extractor   tap-github--custom       meltanolabs 1.0.3   installed
...
### Using `plugin` with Environments

The `plugin` command can accept the `--environment` flag to target a specific [Meltano Environment](https://docs.meltano.com/concepts/environments). The [`default_environment` setting](https://docs.meltano.com/concepts/environments#default-environments) in your `meltano.yml` file will be applied if `--environment` is not provided explicitly.
