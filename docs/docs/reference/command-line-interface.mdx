...
Lists the plugins that Meltano Hub offers. Each plugin is listed once, on the [variant](/concepts/plugins#variants) that Meltano installs when `--variant` is not specified. Pass `--all` to list every variant of every plugin.
