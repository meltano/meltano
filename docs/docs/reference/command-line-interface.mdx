...
When you are [logged in to Meltano Cloud](#cloud), the table has an `UPDATE` column. It shows `↑ available` for each plugin whose lock file differs from the definition that Meltano Cloud serves. Run `meltano add [--plugin-type <type>] <name>` to update the plugin. Meltano also logs a warning when it runs a plugin that has an update.
...
The JSON output reports `update` as `true` or `false`. A plugin that is not checked reports `false`.
