...
The identity provider is the one Meltano Cloud runs, so it is not configurable. The login flow listens on port `9999`, falling back to `9998` when that one is in use; both are registered as callback URLs for the Meltano CLI application.

Only where the session is stored can be changed, with an environment variable or with a key under `cloud.auth` in your [Meltano user configuration file](/guide/configuration):
