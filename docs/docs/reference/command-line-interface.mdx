...
Lists the plugins that Meltano Hub offers. Each plugin is listed once, on the [variant](/concepts/plugins#variants) that Meltano installs when you ask for none. Pass `--all` to list every variant of every plugin, one on each row, with the default named first and marked. A plugin that offers a single variant is never marked, because there is nothing to choose between.

Pass a pattern to narrow the list to the rows whose plugin name or variant contains it, ignoring case. `gitlab` finds `tap-gitlab`, and `meltanolabs` finds every plugin listed on that variant. A row is kept only where the pattern is visible in it, so pair a variant with `--all` to find every plugin that offers it, rather than only those that default to it.

Meltano Hub offers no search of its own, so the pattern is applied here, after the plugins are fetched.

An index that has been fetched is reused for an hour, so a second listing costs no request at all. Pass `--refresh` to fetch one now, such as after a plugin has just been published; the fresh index takes the place of the one it replaces, so later listings see it too. The cache holds only the index that this command reads. Every other command that reads Meltano Hub, including `meltano add`, always fetches.

A count of what matched is printed above the table, and on its own when nothing did. It names the plugin type where you asked for one, so `--plugin-type loader` reports `1 loader matching 'postgres'`.
