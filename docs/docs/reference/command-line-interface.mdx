...
A plugin installed from a repository reports the revision that its `pip_url` asked for, such as a tag or a branch, rather than a version. A plugin whose `pip_url` names no revision, and one whose version cannot be read for any other reason, reports `-`.
