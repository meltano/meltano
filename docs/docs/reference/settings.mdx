...
Install Meltano with PostgreSQL support using one of these options:

```bash
# Recommended for new installations (uses psycopg3)
pip install "meltano[postgres]"

# For existing installations (uses psycopg2)
pip install "meltano[psycopg2]"
```
...
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO meltano_user;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON SEQUENCES TO meltano_user;
```
...
- Additional tables for subscriptions, tokens, and other features
...
- Use a dedicated database user for Meltano operations
