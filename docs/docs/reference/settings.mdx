...
You can use [`meltano config list meltano`](/reference/command-line-interface#config) to list all available settings with their names, [environment variables](/guide/configuration#configuring-settings), and current values.
...
[`meltano config set meltano <setting> <value>`](/reference/command-line-interface#config), which is used in the examples below, will automatically store configuration in `meltano.yml` or `.env` as appropriate.
...
or use [`meltano config list <plugin>`](/reference/command-line-interface#config)
...
Once you log in with [`meltano cloud auth login`](/reference/command-line-interface#auth), Meltano reads the Hub that Meltano Cloud serves at `https://app.meltano.com/api`, which lists the plugins that Meltano supports, maintains and tests. Set `hub_url` to keep reading a Hub of your own instead.
...
# or
meltano config set meltano cli log_config /path/to/logging.yml
```
