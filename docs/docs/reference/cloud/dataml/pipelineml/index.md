...
`triggered_by`    | `string[]` | Pipelines or workspace tasks that will trigger the pipeline on successful completion.<br/>Supported values for workspace tasks (case-insensitive):<ul><li>`deploy` - workspace [deployment]({{site.baseurl}}/api/resources/deployments)</li></ul>
