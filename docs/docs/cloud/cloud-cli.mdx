...
├─────────────────────┼─────────┼──────────┼─────────────────────────────────────┤
│ http://melty+cheese │ webhook │ active   │ [{'events': ['all']}]               │
│ dragon@meltano.com  │ email   │ active   │ []                                  │
╰─────────────────────┴─────────┴──────────┴─────────────────────────────────────╯
```
...
**Notice**
These are all canse sensitive
 
```sh
meltano cloud config notification set webhook --recipient http://test+melty --filter '{"events": ["job_created", "job_succeeded"], "status": ["succeeded"]}' --filter '{ "status": ["failed"]}'
```
The above example sets a webhook notification to send to ﻿http://test+melty if the event is ﻿(job_created OR job_succeeded AND status = succeeded) OR status = failed.
