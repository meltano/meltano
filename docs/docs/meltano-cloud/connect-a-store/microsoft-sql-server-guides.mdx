...
For **Azure SQL Database**, add a server-level firewall rule allowing these addresses:
...
Taps that emit Singer `BATCH` messages using Arrow are loaded natively by `target-mssql`, with no configuration required. These batches load faster than the default row-by-row insert or the Azure Blob Storage staging above, since data is bulk-copied directly into SQL Server instead of being converted record by record. Upsert and append behavior work the same as with any other stream.
