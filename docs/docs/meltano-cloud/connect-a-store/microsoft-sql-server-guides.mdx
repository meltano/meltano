...
The Meltano platform connects from a fixed set of static egress IP addresses,
listed on [Meltano IP Addresses](/meltano-cloud/ip-addresses). Allow all of
them - a pipeline run can originate from any one.

### Create a Firewall Rule

For **Azure SQL Database**, add a server-level firewall rule for each address.
Run the statement below once per address, substituting `<meltano-ip>` and
giving each rule a unique `@name` (for example `AllowMeltano1`,
`AllowMeltano2`):

```sql
EXECUTE sp_set_firewall_rule
    @name = N'AllowMeltano1',
    @start_ip_address = '<meltano-ip>',
    @end_ip_address = '<meltano-ip>';
```

Or using the `az` CLI, again once per address:
...
az sql server firewall-rule create \
  ...
  --server <server> \
  --name AllowMeltano1 \
  --start-ip-address <meltano-ip> \
  --end-ip-address <meltano-ip>
```
...
Taps that emit Arrow-encoded Singer `BATCH` messages are loaded natively by `target-mssql`, with no configuration required. These batches load faster than the default row-by-row insert or the Azure Blob Storage staging above. Upsert and append behavior work the same as with any other stream.

Requires `target-mssql` v0.2.0 or later.
