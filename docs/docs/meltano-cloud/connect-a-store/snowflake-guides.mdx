...
Replace `<meltano-ip-addresses>` with the quoted, comma-separated list from
...
CREATE NETWORK RULE allow_meltano_access_rule
  MODE = INGRESS
  TYPE = IPV4
  VALUE_LIST = (<meltano-ip-addresses>);
...
Taps that emit Arrow-encoded Singer `BATCH` messages are loaded natively by `target-snowflake`, with no configuration required. Set `clean_up_batch_files` to `false` if you need to keep the batch files around for debugging (defaults to `true`).
