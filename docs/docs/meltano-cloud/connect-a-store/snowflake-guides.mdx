...
Taps that emit Singer `BATCH` messages using Arrow (`encoding: {"format": "arrow"}`) are loaded natively by `target-snowflake`, with no configuration required, using the same high-throughput stage-loading path as large batch loads instead of inserting row by row. Set `clean_up_batch_files` to `false` if you need to keep the batch files around for debugging (defaults to `true`).
