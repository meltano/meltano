...
**Restrict network access.** For security, restrict inbound connections to trusted clients. Meltano connects from a fixed set of static egress IP addresses, listed on [Meltano IP Addresses](/meltano-cloud/ip-addresses).

- For ClickHouse Cloud, add each address to the service's IP Access List in `/32` CIDR form.
- Avoid allowing `0.0.0.0/0` unless you have a specific reason to expose the service publicly.
- For self-managed ClickHouse, restrict the Meltano service user to the same addresses in your ClickHouse network configuration.
...
For Meltano connections, allow every address listed on [Meltano IP Addresses](/meltano-cloud/ip-addresses), each in `/32` CIDR form.
...
**Connection refused or timeout.** Check that every one of [Meltano's egress IP addresses](/meltano-cloud/ip-addresses) is allowed by your ClickHouse IP access list and that you are using the correct port: `8443` (secure HTTP), `8123` (plain HTTP), or `9000` (native).
...
**ClickHouse Cloud connection fails.** Make sure TLS is enabled, the port is set to `8443`, and all of [Meltano's IP addresses](/meltano-cloud/ip-addresses) are present in the ClickHouse Cloud IP Access List.
