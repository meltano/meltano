...
| **Sync type** | *Confirm with engineering — not specified in current source material* |
| **Streams** | Configured per site and folder path (not a fixed list) |
| **Custom queries** | Not supported |

> **Note for whoever finalises this page:** the source material this draft was built from (a Microsoft permissions write-up) didn't specify whether syncs are incremental or full-table, or the read-versus-write access. Fill in "Sync type" before this goes live — everything else on this page is accurate as written.
