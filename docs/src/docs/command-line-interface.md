...
All the output generated by this command is also logged inside the [`.meltano` directory](/docs/project.html#meltano-directory) at `.meltano/run/elt/{job_id}/{run_id}/elt.log`. The `run_id` is a UUID autogenerated at each run.
