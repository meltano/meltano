...
::: tip
The preview command [`meltano run`](/docs/command-line-interface.html#run) is a new way to run cross-plugin workflows, including ELT, in a composable manner.
:::
...
Use the `environment` command to manage [Environments](/docs/environments.html#environments) in your Meltano project.
...
Some flags and options supported by [`meltano elt`](/docs/command-line-interface.html#elt) are not yet supported in `meltano run`. For a list of these and a discussion of available workarounds, please see our issue tracker link [#3094](https://gitlab.com/meltano/meltano/-/issues/3094).
:::
