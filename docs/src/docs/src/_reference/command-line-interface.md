...
::: tip
The preview command [`meltano run`](/reference/command-line-interface#run) is a new way to run cross-plugin workflows, including ELT, in a composable manner.
:::
...
::: warning
The run command is a preview feature. Its functionality and CLI signature is still evolving.

During the feature preview, and similar to `meltano invoke dbt:[cmd]`, you may need to perform additional steps to populate `DBT_*` specific environment variables before you are able to directly invoke dbt commands. 
For more information and available workarounds, please see our issue tracker link [#3098](https://gitlab.com/meltano/meltano/-/issues/3098).

Some flags and options supported by [`meltano elt`](/reference/command-line-interface#elt) are not yet supported in `meltano run`. For a list of these and a discussion of available workarounds, please see our issue tracker link [#3094](https://gitlab.com/meltano/meltano/-/issues/3094).
:::
...
::: tip
An `orchestrator` plugin is required to use `meltano schedule`: refer to the [Orchestration](/tutorials/orchestration) documentation to get started with Meltano orchestration.
:::
