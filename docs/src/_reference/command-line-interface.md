...
Multiple commmand blocks can be chained together or repeated, and tap/target pairs will automatically be linked to perform EL work.
If you have an active environment defined, a Job ID is autogenerated for each extractor/loader pair and used to store and look up the [incremental replication state](/guide/integration#incremental-replication-state) in the [system database](/guide/production#storing-metadata).
This allows subsequent runs with the same extractor/loader combination's to pick up right where the previous run left off.
The format of the generated id's is `<environment_name>:<tap_name>-to-<target_name>`. Note that inline mapping names are _not included_ when generating ID's.

If no environment is specified, `meltano run` _does not_ generate an ID, nor does it track state when running without an active environment.
...
When an active environment is present, `run` will attempt to run incrementally/save state by default. However, three top level flags are provided to alter behaviour:
...
- `--force` will force a job run even if a conflicting job with the same generated id is in-flight.

Examples:

```bash
# run the two pipelines in series
# the autogenerated id for the first will be 'dev:tap-gitlab-to-target-postgres'
# the autogenerated id for the second will be 'dev:tap-gitlab-to-target-mysql'
meltano --environment=prod run tap-gitlab hide-secrets target-postgres tap-salesforce target-mysql

# run the pipelines in series, performing a full refresh or all.
...
Note that `meltano run` _does not_ generate a job or preserve state when running without an active environment.
