...
Meltano supports installing dbt in two ways:

- Adding an adapter-specific transformer plugin (e.g. `dbt-snowflake`), in line with dbt's [available Adapters documentation](https://docs.getdbt.com/docs/available-adapters), for use with the new [`run` command]().
- (legacy) Adding the `dbt` transformer plugin, for use with [transform plugins](/concepts/plugins#transforms) and the [`elt` command](). A transform plugin in Meltano is simply a set of dbt models that can be installed as a package.

More details on [migrating](/guide/transformation#migrating-dbt) from a legacy `dbt` install to an adapter-specific one (e.g. `dbt-snowflake`).

## Adapter-specific `dbt` Installation and Configuration
...
meltano --environment=prod config dbt-snowflake
```
...
There are two ways to run adapter-specific dbt plugins using Meltano; in a pipeline using [`meltano run`]() or standalone using [`meltano invoke`]().
...
## (Legacy) `dbt` Installation and Configuration
