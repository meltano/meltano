...
Use the `meltano cloud config env set --key <SECRET_NAME> --value <SECRET_VALUE>` CLI command to set an environment variable secret.

Note: The value can be passed in directly or via an environment variable.
If you have an environment variable set called `TEST_SECRET`, the example to set it would be `meltano cloud config env set --key TEST_SECRET --value $TEST_SECRET`.
