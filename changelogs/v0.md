...
- [#926](https://gitlab.com/meltano/meltano/issues/926) Add step-by-step instructions for using the DigitalOcean one-click installer
...
- [#998](https://gitlab.com/meltano/meltano/issues/998) Update Analyze UI with conditional loading indicator to prevent query generation prior to connection dialects being loaded (this solution is still useful for when inference supersedes our current manual dialect selection solution)
...
- [!806](https://gitlab.com/meltano/meltano/merge_requests/806) Fix filters editing to prevent input for `is_null` and `is_not_null` while also ensuring edits to existing filter expressions types adhere to the same preventative input.
...
- Fixed Analyze button grouped CSS to align as a single row
...
- Add `project_config.yml` to all meltano projects to store consent for anonymous usage tracking and the project's UUID
