...
class JobFinder:
    ...
    def successful(self, session):  # noqa: ANN001, ANN201
        ...
        return session.query(Job).filter(
            (Job.job_name == self.state_id)  # noqa: RUF100, WPS465
            & (Job.state == State.SUCCESS)  # noqa: RUF100, WPS465
    ...
    def with_payload(self, session, flags=0, since=None, state=None):  # noqa: ANN001, ANN201
        ...
        query = (
            ...
            .filter(
                (Job.job_name == self.state_id)  # noqa: RUF100, WPS465
                & (Job.payload_flags != 0)  # noqa: RUF100, WPS465
                & (Job.payload_flags.op("&")(flags) == flags)  # noqa: RUF100, WPS465
                & Job.ended_at.isnot(None)
            )
    ...
    def all_stale(cls, session):  # noqa: ANN001, ANN206
        ...
        return session.query(Job).filter(
            (Job.state == State.RUNNING)  # noqa: RUF100, WPS465
            & (
                (
                    Job.last_heartbeat_at.isnot(None)  # noqa: RUF100, WPS465
                    & (Job.last_heartbeat_at < last_valid_heartbeat_at)
                )
                | (
                    Job.last_heartbeat_at.is_(None)  # noqa: RUF100, WPS465
...
class Job(SystemModel):  # noqa: RUF100, WPS214
