...
class Job(SystemModel):
    ...
    @state.setter
    def state(self, value):  # noqa: ANN001, ANN202
    ...
    def fail(self, error=None):  # noqa: ANN001, ANN201
    ...
    def success(self):  # noqa: ANN201
        """Mark the job as having succeeded."""
        self.ended_at = datetime.now(timezone.utc)
        self.transit(State.SUCCESS)

    def fail_stale(self):  # noqa: ANN201
    ...
    def _heartbeat(self):  # noqa: ANN202
        """Update last_heartbeat_at for this job in the db."""
        self.last_heartbeat_at = datetime.now(timezone.utc)

    async def _heartbeater(self, session):  # noqa: ANN001, ANN202
