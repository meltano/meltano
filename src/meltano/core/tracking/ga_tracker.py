...
class Tracker:
    def __init__(self, tracking_id, request_timeout, client_id):
...
class GoogleAnalyticsAnonymousTracker(Tracker):
    def __init__(self, tracking_id=None, request_timeout=None):
        super().__init__(tracking_id, request_timeout, "package_tracking")

    def track_meltano_ui(self, action, debug: bool = False):
        action = f"meltano ui"
...
class GoogleAnalyticsTracker(Tracker):
    def __init__(
        ...
        super().__init__(tracking_id, request_timeout, client_id or self.project_id())
    ...
    def track_data(self, data: Dict, debug: bool = False) -> None:
        ...
        if self.send_anonymous_usage_stats == False:
            ...
            return
        super().track_data(data, debug=debug)
