...
def _prompt_for_confirmation(prompt):  # noqa: ANN001, ANN202
    ...
    def _prompt_callback(ctx: click.Context, param, value: bool) -> None:  # noqa: ANN001, ARG001
...
@click.pass_context
def list_state(ctx: click.Context, pattern: str | None) -> None:  # noqa: WPS125
...
@pass_project(migrate=True)
@click.pass_context
def get_state(ctx: click.Context, project: Project, state_id: str) -> None:  # noqa: WPS463
    """Get state."""
    state_service: StateService = (
        state_service_from_state_id(project, state_id) or ctx.obj[STATE_SERVICE_KEY]
    )
