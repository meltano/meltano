...
When Meltano maintained Taps and Targets are considered, some implicit assumptions are followed concerning how the source data are stored to the Target Data Store:
...
*  For each Source from which data are extracted through the use of a specific Tap, a schema is created for storing all the data extracted through that Tap. For example, `RAW.SALESFORCE` for data extracted from Salesforce, `RAW.ZENDESK` for data extracted from Zendesk, etc.

*  Each stream generated by a TAP results in a same-named Table created in the schema from that Tap based on the information sent in the Singer.io SCHEMA message for that stream.

*  Schema updates are supported in case the schema of an entity changes in a future run, and in case we receive more than one SCHEMA messages for a specific stream in the same Extract-Load run.

    When a SCHEMA message for a stream is received, our Targets check in the Database provided in the config whether there is already a table for the entity defined by the stream.
    * If there is no such table (or even schema), they are created.
    * If there is already a table for that entity, our Targets create a diff to check if new attributes must be added to the table or existing ones have their type updated and move forward with making the changes.
    ...
    3. Never drop columns, only UPDATE existing ones or ADD new ones
...
composite key (combination of key_properties) then the new record UPDATEs the existing one.
    ...
    For a TARGET to work on append-only mode and the target tables to store historical information, no key_properties must be defined.

*  Even if there is no timestamp_column attribute in the SCHEMA sent to a Target for a specific stream, it is added explicitly. Each RECORD has the timestamp of when the Target received it as a value. As an example, `target-snowflake` sets the name of that attribute to `__loaded_at` when an explicit name is not provided in the Target's configuration file.
