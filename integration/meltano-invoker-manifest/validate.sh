...
output=$(meltano invoke tap-gitlab --version 2>&1)
[ -f .meltano/manifests/meltano-manifest.json ]
...
manifest_content=$(cat .meltano/manifests/meltano-manifest.json)
echo "$manifest_content" | grep -q '"PROJECT_VAR": "project_value"'
echo "$manifest_content" | grep -q '"TAP_GITLAB_VAR": "plugin_value"'
...
touch meltano.yml
sleep 1
output=$(meltano invoke tap-gitlab --version 2>&1 || true)
