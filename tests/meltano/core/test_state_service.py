...
class TestStateService:
    def test_validate_state(self, state_service):  # noqa: ANN001, ANN201
    ...
    def test_get_state(self, state_service, state_ids_with_expected_states):  # noqa: ANN001, ANN201
        for state_id, expected_state in state_ids_with_expected_states:
            assert state_service.get_state(state_id) == expected_state

    def test_list_state(self, state_service, state_ids_with_expected_states):  # noqa: ANN001, ANN201
        assert state_service.list_state() == dict(state_ids_with_expected_states)

    def test_add_state(self, state_service, payloads):  # noqa: ANN001, ANN201
    ...
    def test_set_state(self, jobs, payloads, state_service):  # noqa: ANN001, ANN201
    ...
    @pytest.mark.usefixtures("job_history_session")
    def test_clear_state(self, jobs, payloads, state_service):  # noqa: ANN001, ANN201
        for job in jobs:
            state_service.clear_state(job.job_name)
            assert state_service.get_state(job.job_name) == payloads.mock_empty_payload

    @pytest.mark.usefixtures("job_history_session")
    def test_merge_state(self, jobs, state_service):  # noqa: ANN001, ANN201
    ...
    def test_copy(self, state_ids, state_service):  # noqa: ANN001, ANN201
        state_id_pairs = [
    ...
    def test_move(self, state_ids, state_service):  # noqa: ANN001, ANN201
        state_id_pairs = [
            (state_ids[idx], state_ids[idx + 1])
            for idx in range(0, len(state_ids) - 1, 2)
        ]
