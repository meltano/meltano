...
class TestCliState:
    ...
    def test_state_service_from_state_id_returns_none_non_convention(  # noqa: ANN201
        self,
        project,  # noqa: ANN001
    ...
    def test_state_service_from_state_id_returns_state_service_convention(  # noqa: ANN201
        self,
        project,  # noqa: ANN001
        state_id,  # noqa: ANN001
    ):
    ...
    def test_list(self, state_ids, state_service, cli_runner):  # noqa: ANN001, ANN201
    ...
    def test_list_pattern(  # noqa: ANN201
        self,
    ...
    def test_set_from_string(self, state_service, state_ids, payloads, cli_runner):  # noqa: ANN001, ANN201
        with mock.patch("meltano.cli.state.StateService", return_value=state_service):
    ...
    def test_set_from_file(  # noqa: ANN201, PLR0913
        self,
    ...
    def test_merge_from_string(self, state_service, state_ids, cli_runner):  # noqa: ANN001, ANN201
        with mock.patch("meltano.cli.state.StateService", return_value=state_service):
            job_pairs = [
    ...
    def test_merge_from_file(  # noqa: ANN201
        self,
        tmp_path,  # noqa: ANN001
        state_service,  # noqa: ANN001
        state_ids,  # noqa: ANN001
        ...
        with mock.patch("meltano.cli.state.StateService", return_value=state_service):
            ...
            for job_src, job_dst in job_pairs:
                job_src_state = state_service.get_state(job_src)
                job_dst_state = state_service.get_state(job_dst)
                filepath = os.path.join(tmp_path, f"{job_src}-{job_dst}")  # noqa: PTH118
                with open(filepath, "w+") as state_file:  # noqa: PTH123
    ...
    def test_merge_from_job(self, state_service, state_ids, cli_runner):  # noqa: ANN001, ANN201
        with mock.patch("meltano.cli.state.StateService", return_value=state_service):
            job_pairs = [
                (state_ids[idx], state_ids[idx + 1])
                for idx in range(0, len(state_ids) - 1, 2)
    ...
    def test_copy_over_existing(self, state_service, state_ids, cli_runner):  # noqa: ANN001, ANN201
        with mock.patch("meltano.cli.state.StateService", return_value=state_service):
            job_pairs = [
                (state_ids[idx], state_ids[idx + 1])
                for idx in range(0, len(state_ids) - 1, 2)
    ...
    def test_copy_to_new(self, state_service, state_ids, cli_runner):  # noqa: ANN001, ANN201
        with mock.patch("meltano.cli.state.StateService", return_value=state_service):
    ...
    def test_move(self, state_service, state_ids, cli_runner):  # noqa: ANN001, ANN201
        with mock.patch("meltano.cli.state.StateService", return_value=state_service):
            job_pairs = [
                (state_ids[idx], state_ids[idx + 1])
                for idx in range(0, len(state_ids) - 1, 2)
    ...
    def test_get(self, state_service, cli_runner, state_ids_with_expected_states):  # noqa: ANN001, ANN201
        with mock.patch("meltano.cli.state.StateService", return_value=state_service):
    ...
    def test_clear(self, state_service, cli_runner, state_ids):  # noqa: ANN001, ANN201
        with mock.patch("meltano.cli.state.StateService", return_value=state_service):
            for state_id in state_ids:
    ...
    def test_clear_prompt(self, state_service, cli_runner, state_ids):  # noqa: ANN001, ANN201
        with mock.patch("meltano.cli.state.StateService", return_value=state_service):
            for state_id in state_ids:
